...
import yue.wangtong.lht.tencent.im.viewfeatures.ChatView;
...
public class ChatPresenter implements Observer {
        ...
        synchronized (this) {
            conversation.sendMessage(message, new TIMValueCallBack<TIMMessage>() {
                ...
                public void onError(int code, String desc) {//发送消息失败
                    ...
                    //错误码code含义请参见错误码表
                    Log.i("this", "sendMessageonError:" + code + desc);
     ...
     */
    public void getMessage(@Nullable TIMMessage message, final boolean isFirst) {
        conversation.getMessage(LAST_MESSAGE_NUM, message, new TIMValueCallBack<List<TIMMessage>>() {
            ...
            public void onSuccess(List<TIMMessage> timMessages) {
                if (isFirst) {
                    view.showMessage(timMessages, true);
                } else {
                    view.showMessage(timMessages, false);
                }

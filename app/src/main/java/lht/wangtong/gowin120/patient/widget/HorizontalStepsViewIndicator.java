...
public class HorizontalStepsViewIndicator extends View
{
    ...
    private int mUnCompletedLineColor = ContextCompat.getColor(getContext(), R.color.stepsviewindicator_uncompleted);//定义默认未完成线的颜色  definition
    ...
    private void init()
    {
        ...
        mCircleRadius = 0.3f * defaultStepIndicatorNum;
        //线与线之间的间距    set mLinePadding
        mLinePadding = 1f * defaultStepIndicatorNum;

        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.complted_img);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.drawable.default_img);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.drawable.wrong_img);//未完成的icon
    }
